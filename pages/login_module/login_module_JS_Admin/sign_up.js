    ...
    let employ_no=document.getElementById('employ_no');
    ...
    buttonSubmit.addEventListener('click', function(e){
        ...
        if(validation(data)){
        checkEmail('http://localhost:80/mecl/checkemail_admin',{email:email.value})
        .then((res)=>{
            console.log(res);
            if(res['status']==400){
                ...
                postData('http://localhost:80/mecl/index_admin', orginaData)
                .then((res)=>{
...
function validation(data){
   ...
   if(data.namee.value===""){
    ...
    //     isValidate=false;
    // }
    if(data.employ_no.value===""){
        text="Enter the employ no.";
        data.employ_no.placeholder=text;
        data.employ_no.style.borderColor="red";
        isValidate=false;
    }
    if(data.email.value===""){
    ...
    if(data.employ_no.value!=""){
        data.employ_no.style.borderColor="green";
    }
    if(data.email.value!=""){
        checkEmail();
    }
