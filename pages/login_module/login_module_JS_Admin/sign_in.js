...
signInButton.addEventListener('click',function(e){
    ...
    validation(data);
    if(validation(data)){
        login('http://localhost:80/mecl/login_admin',{email:email.value,password:password.value})
        .then((res)=>{
            if(res['status']==400){
                myDivMsg.innerHTML=res['msg']

                //redirect to jobpage
                function redirect(username,useremail,employ_id){
                 //local storage
                const datatoStore={name:username,email:useremail,employ_id:employ_id}
                localStorage.setItem('admin_details',JSON.stringify(datatoStore));
                window.location.href='/pages/bill_cr/bill_cr.html';
                }
                setTimeout(redirect(res['name'],res['email'],res['employ_id']),2000);
            }else{
