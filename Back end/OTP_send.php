...
switch ($method) {
    case 'POST':
        ...
        $user_total_bill=$user->total_bill;
        ...
        // $user->otp !="" &&   
        if( $user->email != "") {
            $bill_no_check="SELECT * FROM bill_data WHERE bill_no=$user_bill_no;";
            // echo($user_email);

            $stmt= $conn->prepare($bill_no_check);
            $stmt->execute();
            ...
            $user_count=count($users);
                ...
                if($user_count!=0){
                    // $active = 0;
                    $sql = "UPDATE bill_data SET otp='$user_OTP'  WHERE bill_no = '$user_bill_no';";                                         
                    ...
                    try {
                        ...
                        $mail->Subject = "OTP FOR MECL PAYEMNT ";
                        $mail->Body    = "Hi, $user->email.<b>Do not share your OTP with anyone otp<b>
                            <h1> OTP code is :  $user_OTP AMOUNT is: $user_total_bill<h1/> ";
                        $mail->AltBody = 'OTP VERIFICATION FORM MECL ';
                        ...
                        //      echo json_encode($d);
                        $response=['status'=>200,'msg'=> 'Send successfully check your email !'];
