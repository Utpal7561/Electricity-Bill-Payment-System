...
switch ($method) {
    case 'POST':
        $user=json_decode(file_get_contents('php://input'));
            ...
            if($user_name && $user_email && $user_phone && $user_password && $user_uuid && $user_employ_no){
            $sql= "INSERT INTO admin(id, employ_id, mobile_number, password, status, uuid, email, name) 
            VALUES(null, :employ_no, :phone, :password, :status, :uuid, :email, :name)"; 

            $stmt = $conn->prepare($sql);
            $stmt->bindParam( ':employ_no' , $user->employ_no);
            ...
            $stmt->bindParam( ':name' , $user->name);
                ...
                if($stmt->execute()) {
                    ...
                    try {
                        ...
                        $mail->addAddress($user->email, 'Admin');     //Add a recipient
